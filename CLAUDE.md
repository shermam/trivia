...
- **Read `BACKLOG.md`** — the queue of work after the audit: the items, the order and the reasoning for it, what each one has to watch out for, and what was considered and deliberately not queued. If the task is "continue where we left off", that file _is_ the brief. Its working rules are `AUDIT_REMEDIATION.md` §3.
- **Read `AUDIT_REMEDIATION.md`** — an audit produced 55 findings, which grew to 60 as fixing them surfaced more; all are now closed bar one waiting on legal review. **It is a closed record, not a queue** — new work goes in `BACKLOG.md`. Read it anyway before touching an audited area: §4 records decisions taken so they aren't silently revisited, §5 is the register, and §6 explains what each fix actually shipped and what it cost to learn.
...
### 4.0 The published policies describe real behaviour

`/privacy` and `/terms` are the only documents in this repo whose correctness **decays without anyone touching them**. Every factual claim in them was written by reading the source, which is what makes them worth publishing — and what makes a change to the source able to falsify them silently. This already happened once: the pages sat unmerged for 165 commits and arrived claiming the app hot-linked Google Fonts, recorded no author on contributions, and offered no way to delete an account, none of which was true any more (see `AUDIT_REMEDIATION.md` §6, H2).

- **Adding analytics, advertising, a tracking pixel, error reporting, a session recorder or any third-party SDK that observes users falsifies the policy the moment it ships.** The Privacy Policy states outright that there are none of these, that no cookies are set, that nothing is sold or shared, and lists an exhaustive table of every host the browser contacts. Adding Firebase Analytics, `gtag`, Sentry or similar means updating that section, the third-party table, the lawful-basis table and `LEGAL_LAST_UPDATED` **in the same PR** — and probably adding a consent mechanism, since analytics identifiers generally need consent under EU ePrivacy rules and are an active ANPD enforcement theme in Brazil. **Do not pre-announce a practice the app does not yet have**: a policy describing collection that isn't happening is inaccurate in the direction that costs users' trust for nothing, and it buys no legal cover, because a vague forward-looking sentence cannot support processing that has not been specified. **[review-only]**
- **A change to what is stored, for how long, or who can read it changes the policy too** — a new collection, a new field holding personal data, a new TTL, a rules change that widens a read. The retention and data-inventory sections are specific on purpose. **[review-only]**
- **The operating company's name and CNPJ are load-bearing, not decoration** — GDPR Art 13(1)(a) wants the controller identified, and Brazil's e-commerce decree wants the corporate name and CNPJ shown to consumers. They live in `legal.ts` and are pinned by `legal-pages.spec.ts`. **[unit tests]**

Nothing automated can catch the first two. That is precisely why they are written down here rather than assumed — the mechanism is you noticing.
...
- **Every `setTimeout`, `setInterval`, `addEventListener` and `onSnapshot` has a matching teardown**, including inside a `Promise.race` helper that resolves early — racing a listener leaves it attached, receiving and billing for events nobody is waiting on. This app no longer has an `onSnapshot` anywhere (`BACKLOG.md` item 2 replaced both with reads), so the live cases are timers — but the rule is kept naming listeners, because the reason it exists is that the leak is invisible from the outside, and that is a property of the pattern rather than of this codebase. **[review-only]**
- **A timeout should cancel the work, not just stop waiting for it.** `Promise.race` against a timer abandons the request: it runs to completion, and the caller pays for a result it will never read. Use the mechanism the API provides — `AbortSignal.timeout()` for `fetch`, `HttpsCallableOptions.timeout` for Firebase callables, RxJS `timeout()` for `HttpClient` (unsubscribing aborts), a wall-clock bound for a poll (`pollUntil`). Check the SDK before assuming there is one. **The strongest version of this rule is to pick a transport that has one**: Firestore's `getDocs`/`getDoc`/`setDoc`/`addDoc` took no options argument at all, and rather than keep working around that, every Firestore call now goes over `fetch` and genuinely cancels (`BACKLOG.md` item 2). What is left for `giveUpAfter()` is Auth's `signInAnonymously`, which has no alternative, and an IndexedDB read where "stop waiting" is the honest verb. **[review-only]**
...
- **A CSP directive that governs a resource in the page does not govern the same resource fetched by the service worker.** `ngsw-worker.js` responds to every request the page makes — cross-origin included — and re-issues the ones it does not cache as a `fetch()` from inside the worker, where **`connect-src`** applies rather than the `script-src`/`frame-src` the page itself would have used. An origin allowed only by the latter loads perfectly until a service worker takes control of the page, and then stops. Every property of that failure is hostile: the page's CSP is not violated so nothing is reported against it, the refusal is logged in the worker's console rather than the page's, Angular's `safeFetch` turns it into a synthetic `504` that reads like the remote host's fault, and a hard reload — which bypasses the service worker — makes it disappear. It cost weeks of intermittent Google sign-in failures. `npm run csp:verify` now enforces the invariant: every origin allowed for a subresource must also be in `connect-src`, which grants strictly less than being allowed to execute or frame it already does. **[csp:verify]**
