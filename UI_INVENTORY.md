...
**Arriving plays a cue** (unless muted, or in embed mode): a four-note rising fanfare when every question was answered correctly, a shorter falling pair otherwise. It plays on a reload of this screen too.
...
- **No sound plays.** The mute lives in the nav drawer, which is part of the top bar — so an embedded game with audio would be a noise the reader has no way to switch off.
