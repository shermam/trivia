...
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| **Auth not ready yet** (`authReady()` false) | A **skeleton shaped like the resolved chip**, not the word "Loading…" (which rendered 34px tall against 42px for every resolved state and made the chip change height the moment auth settled). At `sm` and above: a pulsing avatar-sized grey circle plus a pulsing skeleton **bar** reserving the label's width (the real label rendered `text-transparent`, so the box keeps the text's metrics) — sized to the anonymous chip, so nothing moves when it resolves. Below `sm` the label region is collapsed, so the skeleton is the **pulsing circle alone** at 42px and the chip widens into "Sign in" — see the animation note below. | "Loading…", `sr-only`, plus `aria-busy` on the trigger                            |
