...
| **Fully authenticated, not yet saved**                                                    | Form: text input (placeholder "Enter your name", prefilled from profile display name, max 30 chars, required) + **"Save Score" button** (disabled while saving or while name is blank; label → "Saving…" while in flight), then a labelled **"Country to rank in"** dropdown listing every country by name in the reader's own locale, with "Prefer not to say" first, and a hint reading "Published beside your name and score on that country's public board, as well as the global one. Leave it unset and only the global board gets your score." It opens on the country the app inferred, or on the one the reader last chose |
