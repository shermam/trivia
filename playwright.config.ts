...
export default defineConfig({
  ...
  /**
   * Two preview-only specs, and this runner has nothing for either to test:
   * `ng serve --configuration=e2e` sets no `serviceWorker` in `angular.json`,
   * so no `ngsw-worker.js` and no `ngsw.json` are emitted for a worker to
   * install from, and a dev server sends none of `firebase.json`'s headers.
   * `service-worker-oauth-origins.spec.ts` needs those headers;
   * `offline-play.spec.ts` needs the precache the worker installs. Both run
   ...
   */
  testIgnore: ['**/service-worker-oauth-origins.spec.ts', '**/offline-play.spec.ts'],
