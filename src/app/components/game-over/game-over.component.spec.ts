...
describe('GameOverComponent answer recap (FEAT-001)', () => {
  ...
  /*
   * The over-long case, and it is here because it is the *only* input that
   * separates the length check from the per-entry one. A history shorter than
   * the round leaves `history[i]` undefined for the tail, which no answer id
   * matches, so the per-entry guard already returns `[]` — delete the length
   * check and every partial-history test above still passes. A history longer
   * than the round is the reverse: every entry resolves, so only the length
   * check refuses it. Found by mutation: without this test, `history.length
   * !== questions.length` can be removed with the suite still green.
   */
  it('renders no recap when the history is longer than the round', () => {
    const { query } = render({
      questions: [q0],
      answerHistory: ['q0:right', 'q1:right'],
    });
