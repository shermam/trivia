...
export const appConfig: ApplicationConfig = {
  providers: [
    ...
    provideServiceWorker('ngsw-worker.js', {
      // Also off under navigator.webdriver (true for Playwright/Selenium, never for a
      ...
      // gated off the same way (the tests in profile.spec.ts went from ~7s combined to 46s+
      // with a timeout, sign-in-save-score.spec.ts from ~55s to 1m+ with a timeout, confirmed
      // against a clean baseline run with no PWA changes at all) — installing/activating the
      // worker and its integrity-checking the precached app shell is real,
      // uninteresting-to-these-specs work competing for the same main thread and network as
      // Firebase's own calls.
