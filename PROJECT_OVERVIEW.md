...
- **`TriviaService.getQuestions()` falls back to the offline pool** when `navigator.onLine` is `false`, or when the network fetch itself throws (`TriviaService.playingOffline` signal reflects which happened) — a legitimate "no questions matched this filter" *result* (empty array, no error, e.g. a very narrow category/difficulty combo) is left alone, only a failed/skipped network attempt triggers the fallback. `OfflineQuestionsService.getOfflineQuestions()` prefers questions matching the requested category/difficulty, but falls back to the whole pool if too few match — a mismatched offline game beats no offline game at all.
...
- **Disabled under the `e2e`/`lighthouse` Angular build configurations** (`environment.enableOfflinePrefetch`, `false` only in `environment.e2e.ts`): the background prefetch's own `opentdb.com` request would otherwise race Cypress's `cy.wait('@questions')` intercepts (`cypress/support/commands.ts`) and could satisfy the wrong wait, and it isn't something Lighthouse's audited page load needs competing for resources. See the comment on that flag for the full reasoning.
