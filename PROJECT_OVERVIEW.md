...
App icons (`public/favicon.svg`/`.ico`, `public/apple-touch-icon.png`, linked from `index.html`) are the Trivimind brain/lightbulb/question-mark mark recolored into an Emerald-family green (`#0fa968`) rounded-square background with the brain/bulb/`?` silhouette in solid white on top. The original source art defines that silhouette as literal transparent cutouts in a single navy path — fine on a white canvas, but would show whatever's behind it (e.g. bleed through to a dark tab background) if used as-is — so `favicon.svg` layers a white backing `<rect>` sized to fully cover the icon's silhouette behind the recolored path, making the interior fully opaque. `favicon.ico` embeds six sizes (16 up to 256px), each rasterized directly from the vector rather than resized down from a single bitmap — the intricate line art (e.g. the small curl details at the top of the brain) needs at least ~128px to stay legible, so anything that renders the ICO's largest available frame (browsers, OS/file-manager previews, GitHub's image-diff view) still shows full detail instead of a blurred/merged blob.

The same mark is also used in-app as the brand badge — top bar logo, `GameSetupComponent`'s hero icon, and the Pro card badge on `/pricing` — via a dedicated `LogoComponent` (`src/app/components/logo/logo.component.ts`, selector `app-logo`) that inlines the identical SVG (own baked-in emerald background + white silhouette, byte-identical path data to `favicon.svg`). It's kept separate from the shared `IconComponent`'s single-color 24×24 stroke-icon `@switch` (same convention as `ProviderIconComponent`'s OAuth brand marks) since it's a fixed multi-part filled mark, not a `currentColor` stroke glyph — callers just set `[size]` and, where the original design had a drop shadow, a `rounded-2xl shadow-cta` class on the element itself (no wrapping background box needed, since the mark already draws its own rounded-square fill). The purely decorative sparkle accents elsewhere (game-setup's "Start Game" button, pricing's "Subscribe" button, add-question's hero badge) still use `IconComponent`'s `sparkles` icon — only the three brand-badge placements above were switched.
