...
| [`docs/dev-environment.md`](docs/dev-environment.md) | **The three environments** — local emulators, the `trivimind-dev` project, production — why `npm start` used to run against production, and the by-hand steps to finish the dev project | You are setting a machine up, or doing the `trivimind-dev` setup                                  |
