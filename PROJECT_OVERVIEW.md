...
Two static documents, reachable from a persistent site `FooterComponent` (a sibling of `<router-outlet>` alongside `TopBarComponent`, hidden in embed mode for the same reason). Both render through a shared `LegalPageComponent` shell so neither can drift from the other or lose its attribution line, with the document bodies projected in via `<ng-content>` — which keeps those two templates pure prose and reviewable as copy rather than as markup.

- **The data-handling sections were written from source, not from a template.** Every factual claim — that an anonymous account is created before any interaction, that the leaderboard is world-readable, that the app sets no cookies, that only an email address and a uid reach Stripe, that there are no analytics of any kind — was derived by reading the code that makes it true. This is the point of writing a policy this way: a policy that describes practices the app doesn't have is itself a misstatement about data handling.
- **Everything requiring legal judgement is marked, not guessed.** `ReviewRequiredComponent` (`<app-review-required>`, `role="note"`) renders a loud amber callout around each unresolved item; there are 13 across the two documents (retention, data-subject rights, children/COPPA, storage region, refunds, liability, jurisdiction, contact address, contributed-content licence). Grep for `app-review-required` to enumerate them.
- **`LEGAL_IS_DRAFT` (`legal.ts`) gates a page-level "Draft — not legally reviewed" banner.** Flipping it to `false` is the deliberate act of asserting a human has reviewed the text; `LEGAL_LAST_UPDATED` is set at the same time. Until then, no visitor can mistake these for reviewed policy.
- **Structure and tone are adapted from [Basecamp's open-sourced policies](https://github.com/basecamp/policies) under CC BY 4.0**, which requires attribution — rendered at the foot of both pages. Basecamp's were chosen over Automattic's equivalent specifically because Automattic's are CC BY-**SA**, whose share-alike term would have obliged these pages to carry the same licence.
...
Two findings from the audit series bear directly on these pages: the Google Fonts row in the privacy policy's third-party table is a live GDPR exposure rather than a mere disclosure (self-hosting removes it), and several retention/rights sections cannot be truthfully completed until account deletion exists.
...
- The `/privacy` and `/terms` pages (§1.9) exist but are **drafts** — `LEGAL_IS_DRAFT` is `true` and 13 `<app-review-required>` items are unresolved, covering retention periods, data-subject rights, children/COPPA, storage region, refunds, liability, governing law, a contact address, and the licence for contributed questions. Several of them are blocked on features rather than on legal review: the retention and rights sections can't make a truthful promise until account deletion and data export exist, and the terms' contributed-content section can't promise removal on request while `custom_questions` records no author.
- The app hot-links the Inter typeface from Google Fonts (`fonts.googleapis.com`/`fonts.gstatic.com`, §2.1), which transmits every visitor's IP address to Google before any consent. LG München I (case 3 O 17493/20) held that this specific arrangement breaches the GDPR. Self-hosting the font removes the transfer and should also improve the performance score by dropping two cross-origin preconnects and a render-blocking stylesheet from the critical path. §2.1 currently justifies the hot-link on the grounds that Lighthouse is the guardrail — Lighthouse audits no privacy dimension at all, so that reasoning doesn't cover this risk.
