...
| [`docs/dev-environment.md`](docs/dev-environment.md) | **The three environments** — local emulators, the `trivimind-dev` project, production — how each is addressed, what keeps them apart, and the by-hand console steps the dev project needs | You are setting a machine up, or doing the `trivimind-dev` setup                                  |
...
- **A stale document is a bug.** `CLAUDE.md` §2 requires the relevant file here to be updated in the same PR as the change it describes, and §2a requires that update to be made _in place_ — rewrite the sentence that is now wrong rather than appending a correction to it. Why the change was made goes in the pull request description.
- **What to build next is not in this repo.** The roadmap lives in `shermam/trivia-project`: `NEXT-UP.md` for what is ready to pick up, `features/` for one spec per item. `docs/known-gaps.md` §6 records what is open in the product; the roadmap decides what is done about it.
...
| `AUDIT_REMEDIATION.md`     | The 60-finding audit and the PR series that closed it: status, the findings register, and a narrative per finding                             |
