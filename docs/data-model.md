...
Background pre-creation spends from the same ten, which is why the client rations itself: **at most three of them per window may be pre-created**, and a reload reuses the session already in `sessionStorage` rather than creating another (`app.md` §1.6). Both bounds exist so that a reader refreshing a page they are thinking about, or flicking the currency switch while they decide, still has slots left for the click they eventually make.
