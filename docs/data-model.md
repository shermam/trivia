...
- **Update / Delete**: none from the client (`allow update, delete: if false`) — console-only for now. This is what item 4's reviewer capability will change, and it is why `status` is inert until then: the field exists, and nothing can move it.
...
**`status` — the moderation field, in place but inert** (`BACKLOG.md` item 4b).

`firestore.rules`' `statusOnSubmission()` accepts `'approved'` and nothing else on create, and there is still no update path at all, so today every document in the bank is approved and the field changes no behaviour. It exists now because putting it in place is a migration and flipping it is not: widening an exact-key `hasOnly()` allowlist and backfilling every existing document is the risky half, and it is worth landing on its own, with nothing else moving.
...
`firestore.rules` is the app's real security boundary, so it has a dedicated unit suite (`npm run rules:test`, `firestore-tests/`, 229 tests across six spec files) built on `@firebase/rules-unit-testing` and run against the Firestore emulator. Deliberately outside `src/` and driven by its own `vitest.rules.config.ts`, so the Angular build, `ng test` and the ESLint globs never pick it up.
