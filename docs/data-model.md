...
**Source attribution (`sourceUrl` / `sourceTitle`) is optional, and optionality is what makes it safe to add** (`FEAT-022`). Every other field this collection has gained arrived as a migration, because the read rule or the client filter dereferenced it — `status` needed `scripts/backfill-question-status.mjs` run between two deploys precisely because `resource.data.status == 'approved'` cannot match a document that has no `status`. Nothing reads these two. No rule branches on them, no query filters on them, and the readers render nothing when they are absent. So widening the `hasOnly()` allowlist is the entire migration: existing documents stay valid, and there is no ordering hazard because there is no step that could run in the wrong order. **The precondition recorded for attribution — "verify the bank holds nothing worth migrating" — does not generalise to optional fields; it was a property of `status` being dereferenced, not of adding a field.**

Both are validated on create and neither can be added afterwards. `sourceUrl` must be a string beginning `https://` and longer than that prefix, at most 500 characters; `sourceTitle` must be a non-empty string of at most 200. Either may be present without the other: a book has a title and no URL, and a bare link with no publication name is still a link worth following. `http://` is refused rather than upgraded — the CSP would not load the page, so the rule refuses what the reader could not open anyway — and the form catches the same cases first (`app.md` §1.4) so the contributor gets a named field error instead of a bare `permission-denied`.

**The reviewer's update rule was deliberately left alone**, and that is load-bearing rather than an oversight. It still reads `hasOnly(['status'])`, so a reviewer approving a question cannot attach, alter or remove a citation while doing it — attribution stays the author's word, the same way `createdBy` does. A rules test asserts the refusal, because widening that allowlist is a one-line change that would look like a convenience.

**Readers re-check the URL rather than trusting the stored value** (`SourceLinkComponent`, `app.md` §1.4). `CLAUDE.md` §4.4 asks the reader to be right regardless of the writer: the rule is one deploy from being widened, Firestore is a public API, and a stored value that is not a usable `https:` URL degrades to plain text instead of rendering a dead anchor.
...
`firestore.rules` is the app's real security boundary, so it has a dedicated unit suite (`npm run rules:test`, `firestore-tests/`, 270 tests across seven spec files) built on `@firebase/rules-unit-testing` and run against the Firestore emulator. Deliberately outside `src/` and driven by its own `vitest.rules.config.ts`, so the Angular build, `ng test` and the ESLint globs never pick it up.
