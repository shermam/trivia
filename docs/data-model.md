...
- **The app's read is one bounded, ordered page** — `createdAt` descending, `REPORTS_PAGE_SIZE` (25) at a time, growing by a page when the reviewer asks for more (`CLAUDE.md` §4.1). There is no `where` to add: the queue wants the newest complaints, whatever they are about, and the ordering is what makes the bound mean something. `createdAt` descending rides the automatic single-field index, so this needs no composite and brings none of D3's deploy risk. Every document has the field — the create rule refuses one without it — which matters because a document missing an ordered field is invisible to the query rather than merely last.
...
- Rules live in `firestore.rules` (`isValidQuestionReport`), with their own suite (`firestore-tests/question-reports.rules.spec.ts`, 30 tests, mutation-verified — nine deliberate rule breaks produced 5/3/1/1/1/3 targeted failures on the create clauses and 4/3/3 on the read: denying every read fails the four accept rows, opening it to any signed-in caller fails the three denial rows, and splitting it into `allow get` + `allow list: if false` fails the three list accept rows — the distinction `CLAUDE.md` §4.6 records the worked example of).
