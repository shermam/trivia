...
- **Never commit a Firebase config/API key to source.** Firebase Hosting auto-serves a reserved `/__/firebase/init.json` endpoint for whatever project is serving the current origin — fetch that at runtime instead of hardcoding config. One bundle then runs against any project, which is what makes a faithful dev environment cheap.
- **…but do not proxy that endpoint to production in local dev, which is the obvious way to do it and is how this project shipped a real defect.** Pointing `proxyConfig` at the live Hosting URL gives `ng serve` a real config, as intended — and a local dev server then reads and writes the **production** database. It fails silently in the worst way: the app works. Point local development at emulators instead (a build-time `useEmulators` flag that skips the fetch entirely, so there is no request left to misdirect), point the proxy at a _dev_ project for the cases that need a real backend, and enforce both with a check — `scripts/verify-environment-isolation.mjs` here. The generalisable shape: **a dev/prod isolation mistake produces no error, only quiet damage**, so it needs a guardrail rather than a convention.
...
- **Entitlement = a custom claim on the Auth token** (e.g. `stripeRole: 'pro'`), set by your webhook handler once a subscription's price carries a role marker in its Stripe metadata. Firestore rules check the claim directly. A separate client-side mirror of subscription state is fine for UX (so the UI reacts before the next token refresh) but should never be the actual security gate — the claim is. **A real-time listener is one way to build that mirror and is rarely worth its price**: this project reached for `onSnapshot` first and later replaced it with a read, because two listeners were the only thing keeping a 559 kB SDK in the bundle. Reads plus a bounded poll after checkout cost less, bill less, and lose only cross-tab immediacy.
- **Client-initiated, function-completed handshake** for anything requiring a live Stripe API call (Checkout session, Billing Portal session): the client creates a small placeholder document (e.g. `customers/{uid}/checkout_sessions/{id}`), a Firestore-triggered function (`onDocumentCreated`) does the real Stripe API call and writes the result (URL or error) back onto that same document, and the client waits for that write. This avoids ever giving the client a callable-function-only path that needs its own separate auth wiring, and keeps the Stripe secret key entirely server-side. **Waiting does not require a listener** — a poll bounded on the wall clock costs a few dozen reads on an action a user takes rarely, and it cannot leak the way a listener raced against a timeout can. Checkout is also the one place where the _redirect back_ from Stripe races your own webhook, so whatever mechanism you pick has to tolerate arriving before the subscription document does.
...
- **If you ship a service worker, your CSP has to be written for two consumers, not one.** A service worker responds to every fetch the page makes and re-issues the ones it does not cache from inside the worker — and there, every request is a connection, so **`connect-src`** governs it rather than the `script-src`/`frame-src`/`img-src` the page would have used. The CSP is usually applied to `**`, which is what puts it on the worker script too. The practical rule: **every origin allowed for a subresource must also appear in `connect-src`** — subresource being the operative word, since a cross-origin iframe or popup is a navigation and is matched to a service worker by the _target_ origin, so the embedding page's worker never touches it. That grants strictly less than what already allows it — fetching bytes from an origin you can already execute scripts from is not a widening. Skip it and you get a resource that works until a service worker takes control of the page, with a CSP refusal logged in the worker's console rather than the page's, surfacing as a synthetic `504` that looks like the remote host's fault, and disappearing on a hard reload because that bypasses the worker. This repo lost weeks of intermittent Google sign-in failures to exactly that, and now enforces the invariant in CI.
- **And a service worker keeps the CSP it was installed with, so fixing the policy does not fix the users.** A worker's policy container is populated from the headers on its _script_ response at install time. The update algorithm compares the script byte-for-byte and installs nothing when it matches — and a framework's worker script (Angular's `ngsw-worker.js`, Workbox's generated `sw.js` when its precache manifest is unchanged) usually does match, because it is a static file. So a header-only fix ships, looks correct in every check, works on your machine because your profile installed the worker after the deploy, and silently does nothing for everyone who visited before it. This repo shipped exactly that fix and had to diagnose the same bug twice. **If your CSP can change, make the worker script's bytes depend on it** — append a fingerprint of the policy at build time. A build timestamp also works but re-installs the worker on every deploy; a fingerprint re-installs it only when the thing that matters changed.
- **A CSP checker that compares the policy against itself has a blind spot exactly where the worst bugs live.** It can only flag an origin already written somewhere in the policy; an origin your app requests that appears in _no_ directive is invisible to it, and that is the more common mistake. Keep a hand-maintained list of the origins the app actually calls at runtime and assert those too — nothing can derive it, because the URLs are built inside third-party SDKs. This repo found `httpsCallable`'s `{region}-{project}.cloudfunctions.net` host missing that way, which had been quietly refusing account deletion and data export in production while every local and emulator run passed.
